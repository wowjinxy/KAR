...
internal static class KarCliTextWriter
{
    ...
    public static void PrintProjectSession(KarProjectSession session)
    {
        ...
        Console.WriteLine("Registries: archive-definition-rules=" + session.ArchiveDefinitionRuleCount + " data-definition-providers=" + session.DataDefinitionProviderCount + " file-kind-classification-rules=" + session.FileKindClassificationRuleCount + " file-kinds=" + session.FileKindCount + " file-handlers=" + session.FileHandlerCount + " resource-handlers=" + session.ResourceHandlerCount + " resource-adapters=" + session.ResourceAdapterProviderCount + " relationship-providers=" + session.RelationshipProviderCount + " actions=" + session.ResourceActionDefinitionCount + " operation-domains=" + session.OperationDomainRuleCount + " context-domains=" + session.DomainContextProviderCount + " workflow-providers=" + session.ToolkitWorkflowProviderCount + " presets=" + session.OperationPresetDefinitionCount);
    ...
    public static void PrintProjectToolkitRegistryCatalog(KarProjectToolkitRegistryCatalog catalog)
    {
        ...
        Console.WriteLine("File kind classification rules:");
        foreach (KarFileKindClassificationRule rule in catalog.FileKindClassificationRules)
            Console.WriteLine("  " + rule.Id + " - " + rule.DisplayName);
        ...
        Console.WriteLine("Relationship providers:");
        foreach (KarProjectRelationshipProvider provider in catalog.RelationshipProviders)
        ...
        Console.WriteLine("Domain context providers:");
        foreach (KarProjectDomainContextProvider provider in catalog.DomainContextProviders)
            Console.WriteLine("  " + provider.Id + " list=" + provider.ListCommand + " context=" + provider.ContextCommand + " - " + provider.DisplayName);

        Console.WriteLine("Toolkit workflow providers:");
        foreach (KarProjectToolkitWorkflowProvider provider in catalog.ToolkitWorkflowProviders)
            Console.WriteLine("  " + provider.Id + " - " + provider.DisplayName);
