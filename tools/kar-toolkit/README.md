...
Use `KarProjectSession.Open`, `KarProject.OpenSession`, `KarProject.CreateSession`, or the `session` CLI command when a frontend or script wants one reusable toolkit entry point with project metadata, explicit source/output workspace policy, a toolkit surface, grouped workflows, and mod output state.
