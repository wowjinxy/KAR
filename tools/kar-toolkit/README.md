...
Project-wide resource inventory, address resolution, byte reads, safe output exports, safe imports, and resource-address scalar edits go through `KarProject.ResourceService`; the `resources`/`resource` CLI commands expose mixed file, HSD root, and A2D entry resources, `export-resource` stages file/root archives or A2D sidecars under the configured output folder, `import-resource` writes file replacements or same-size A2D entry replacements only into output packages, and `set-resource-scalar` edits HSD root resources by address.
