...
{
    HSD_ObjAllocInit(&hsdFObj_alloc_data, sizeof(HSD_FObj), 4);
}
...
void HSD_FObjStopAnimAll(HSD_FObj* fobj, void* obj, HSD_ObjUpdateFunc obj_update,
                         f32 rate)
{
...
static inline u32 FObjLoadWait(HSD_FObj* fobj)
{
    ...
    assert_line(0x17B, st == FOBJ_LOAD_WAIT);
...
{
    u32 st = HSD_FObjGetState(fobj);
    assert_line(0x18E, st == FOBJ_LOAD_DATA0 || st == FOBJ_LOAD_DATA);
...
{
    u32 st = HSD_FObjGetState(fobj);
    assert_line(0x1A2, st == FOBJ_LOAD_DATA0 || st == FOBJ_LOAD_DATA);
...
{
    u32 st = HSD_FObjGetState(fobj);
    assert_line(0x1B6, st == FOBJ_LOAD_DATA0 || st == FOBJ_LOAD_DATA);
...
{
    u32 st = HSD_FObjGetState(fobj);
    assert_line(0x1C8, st == FOBJ_LOAD_DATA0 || st == FOBJ_LOAD_DATA);
...
{
    u32 st = HSD_FObjGetState(fobj);
    assert_line(0x1DB, st == FOBJ_LOAD_DATA0 || st == FOBJ_LOAD_DATA);
...
{
    u32 st = HSD_FObjGetState(fobj);
    assert_line(0x1F8, st == FOBJ_LOAD_DATA0 || st == FOBJ_LOAD_DATA);
...
{
    HSD_FObj* new = HSD_ObjAlloc(&hsdFObj_alloc_data);
    assert_line(0x302, new);
...
{
    HSD_ObjFree(&hsdFObj_alloc_data, fobj);
}

u16 HSD_FObjGetFlags(HSD_FObj* fobj)
{
    if (fobj != NULL) {
        return *(u16*) &fobj->length;
    }
    return 0;
}
