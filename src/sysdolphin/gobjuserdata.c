...
extern void HSD_GObjPLinkChange_Internal(s32 type, HSD_GObj* gobj, u8 p_link, u8 p_prio,
                                         HSD_GObj* position);
extern void HSD_GObjProcRemove(HSD_GObjProc* proc);
...
void HSD_GObjProcRunAll(void)
{
    ...
    for (i = 0; i <= hsdGObj_p_link_max.gproc_pri_max; i++) {
        ...
        while (proc != NULL) {
            ...
            if (proc->flags_3 != lbl_805DE344) {
                proc->flags_3 = lbl_805DE344;
                gobj = proc->gobj;
        ...
        if (mask & 1) {
            ...
            while (prios != 0) {
                if (prios & 1) {
                    ...
                    for (cur = lbl_805DE32C[j]; cur != NULL; cur = cur->next_gx) {
...
{
    HSD_GObj* saved;
    HSD_GObj* cur = lbl_805DE32C[hsdGObj_p_link_max.gx_link_max + 1];
...
void GObj_Init(HSD_GObjLibInitDataType* init_data)
{
    ...
    lbl_805DE334 = HSD_Alloc(sizeof(HSD_GObj*) * (init_data->p_link_max + 1));
    lbl_805DE330 = HSD_Alloc(sizeof(HSD_GObj*) * (init_data->p_link_max + 1));
    for (i = 0; i < init_data->p_link_max + 1; i++) {
        lbl_805DE334[i] = lbl_805DE330[i] = NULL;
    }

    lbl_805DE32C = HSD_Alloc(sizeof(HSD_GObj*) * (init_data->gx_link_max + 2));
    lbl_805DE328 = HSD_Alloc(sizeof(HSD_GObj*) * (init_data->gx_link_max + 2));
    for (i = 0; i < init_data->gx_link_max + 2; i++) {
        lbl_805DE32C[i] = lbl_805DE328[i] = NULL;
    }

    lbl_805DE348 = HSD_Alloc(sizeof(HSD_GObjProc*) * (init_data->gproc_pri_max + 1));
    for (i = 0; i < init_data->gproc_pri_max + 1; i++) {
        lbl_805DE348[i] = NULL;
    }

    lbl_805DE34C = HSD_Alloc(sizeof(HSD_GObjProc*) *
