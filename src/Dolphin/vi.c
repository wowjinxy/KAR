...
static const char* __VIVersion = "<< Dolphin SDK - VI\trelease build: Sep  5 2002 05:33:13 (0x2301) >>";
...
static inline int VISetRegs(void) {
    s32 regIndex;
...
void __VIRetraceHandler(u32 interrupt, OSContext* context) {
...
void __VIInit(VITVMode mode) {
    ...
    *(u32*)OSPhysicalToCached(0xCC) = tv;
    ...
    if (mode == VI_TVMODE_NTSC_PROG || mode == 26 || mode == 3) {
        __VIRegs[1] = (tv << 8) | 5;
        __VIRegs[54] = 1;
    } else {
...
void VIInit(void) {
    ...
    OSRegisterVersion((char*)__VIVersion);
    IsInitialized = TRUE;
...
static inline void setPicConfig(u16 fbSizeX, VIXFBMode xfbMode, u16 panPosX, u16 panSizeX, u8* wordPerLine, u8* std, u8* wpl, u8* xof) {
...
static inline void calcFbbs(u32 bufAddr, u16 panPosX, u16 panPosY, u8 wordPerLine, VIXFBMode xfbMode, u16 dispPosY, u32* tfbb, u32* bfbb) {
    u32 bytesPerLine;
    u32 xoffInWords;
    u32 tmp;
...
void fn_803DE1B8(SomeVIStruct* horVer, u32* tfbb, u32* bfbb, u32* rtfbb, u32* rbfbb) {
    u32 shifted;
...
void fn_803DE48C(u16 dispPosY, u16 dispSizeY, u8 equ, u16 acv, u16 prbOdd, u16 prbEven, u16 psbOdd, u16 psbEven, BOOL black) {
    u16 actualPrbOdd;
    u16 actualPrbEven;
    u16 actualPsbOdd;
    u16 actualPsbEven;
...
void VIConfigure(const GXRenderModeObj* rm) {
    ...
    mode = (HorVer.tv << 2) + HorVer.nonInter;
    tm = getTiming(mode);
    HorVer.timing = tm;
    ...
    if (FBSet != 0) {
        #pragma dont_inline on
        fn_803DE1B8(&HorVer, &HorVer.tfbb, &HorVer.bfbb, &HorVer.rtfbb, &HorVer.rbfbb);
        #pragma dont_inline reset
    }
    #pragma dont_inline on
    fn_803DE48C(HorVer.AdjustedDispPosY, HorVer.AdjustedDispSizeY, tm->equ, tm->acv, tm->prbOdd, tm->prbEven, tm->psbOdd, tm->psbEven, HorVer.black);
    #pragma dont_inline reset
    OSRestoreInterrupts(enabled);
}

void fn_803DEE54(u16 xOrg, u16 yOrg, u16 width, u16 height) {
    BOOL enabled;
    VITiming* tm;
    ...
    #pragma dont_inline on
    fn_803DE48C(HorVer.AdjustedDispPosY, HorVer.DispSizeY, tm->equ, tm->acv, tm->prbOdd, tm->prbEven, tm->psbOdd, tm->psbEven, HorVer.black);
    #pragma dont_inline reset
    OSRestoreInterrupts(enabled);
}
...
u32 VIGetNextField(void) {
    ...
    nextField = (((((vcount - 1) << 1) + ((hcount - 1) / CurrTiming->hlw)) < CurrTiming->nhlines) ? 1 : 0) ^ 1;
    OSRestoreInterrupts(enabled);
...
char lbl_804FC910[0x48] = "<< Dolphin SDK - AI\trelease build: Sep  5 2002 05:34:25 (0x2301) >>";
char lbl_804FC958[0x48] = "<< Dolphin SDK - AR\trelease build: Sep  5 2002 05:34:27 (0x2301) >>";
char lbl_804FC9A0[0x48] = "<< Dolphin SDK - ARQ\trelease build: Sep  5 2002 05:34:29 (0x2301) >>";
char lbl_804FC9E8[0x48] = "<< Dolphin SDK - CARD\trelease build: Sep  5 2002 05:35:20 (0x2301) >>";
