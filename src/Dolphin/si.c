...
static u8 unk_804FCBE0[0x44];
static volatile SIControl Si;
static s32 Type[4];

static SISamplingInfo XYNTSC[12];
static SISamplingInfo XYPAL[12];
static char UnknownFormatMsg[0x148];
...
BOOL __SITransfer(s32 chan, void* outBuf, s32 outLen, void* inBuf, s32 inLen, SITransferCallback callback)
{
    ...
    if (Si.curChan == -1) {
        ...
        comcsr = (comcsr & 0xBF8080F9) | ((callback != 0) << 30) | ((outLen & 0x7f) << 16) |
                 ((inLen & 0x7f) << 8) | ((chan << 1) & 6);
        SICOMCSR = comcsr | 1;
...
#pragma dont_inline on
u32 SISetXY(s32 x, s32 y)
{
    u32 poll;
    BOOL enabled = OSDisableInterrupts();

    poll = (Si.pollValue & 0xFC0000FF) | (x << 16) | (y << 8);
    SIPOLL = poll;
    Si.pollValue = poll;
...
u32 SIEnablePolling(u32 mask)
{
    ...
    {
        BOOL enabled = OSDisableInterrupts();
        poll = (Si.pollValue & ~(mask >> 28)) | ((mask >> 24) & ((mask >> 28) | 0x3fffff0));
        SISR = 0x80000000;
        SIPOLL = poll;
        Si.pollValue = poll;
        OSRestoreInterrupts(enabled);
...
static void GetTypeCallback(s32 chan, u32 status, OSContext* context)
{
    ...
    u32 oldPending;
    u32* wsSlot = (u32*)((u8*)Packet + chan * 0x14 + 0x1F0);
    SIGetTypeCallback* queue;
    s32 i;

    (void)context;

    Type[chan] = (Type[chan] & ~0x80) | status;
    TypeTime[chan] = __OSGetSystemTime();
    ...
    if ((status & 0xF) == 0 && (Type[chan] & 0x18000000) == 0x08000000 && (Type[chan] & 0x80000000) != 0 &&
        ...
        if (oldPending != 0 && (wid & 0x00100000) != 0) {
            *wsSlot = (wid & 0x00CFFF00) | 0x4E100000;
            Type[chan] = 0x80;
            SITransfer(chan, wsSlot, 3, &Type[chan], 3, GetTypeCallback, 0);
            return;
        }
        ...
        if (Type[chan] & 0x00100000) {
            if ((wid & 0x00CFFF00) != (Type[chan] & 0x00CFFF00)) {
                if (!(wid & 0x00100000)) {
                    ...
                    OSSetWirelessID(chan, (u16)(wid >> 16));
                }
                *wsSlot = wid | 0x4E000000;
                Type[chan] = 0x80;
                SITransfer(chan, wsSlot, 3, &Type[chan], 3, GetTypeCallback, 0);
                return;
            }
